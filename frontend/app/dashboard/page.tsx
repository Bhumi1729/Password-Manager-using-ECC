...
const Dashboard: React.FC = () => {
  ...
  // Add useMediaQuery hook to detect screen size
  ...
  useEffect(() => {
    const fetchPasswords = async (): Promise<void> => {
      ...
      try {
        ...
        // Process the passwords - add default strength values
        // Since the API doesn't return passwords in the list view for security
        const processedPasswords = result.data.map((pwd: any) => ({
          ...pwd,
          password: '••••••••••••••••', // Placeholder for security
          strength: Math.floor(Math.random() * 30) + 70, // Random strength between 70-100 as placeholder
          favorite: false // Default not favorite
        }));
        
        setPasswords(processedPasswords);
      } catch (error) {
        console.error('Error fetching passwords:', error instanceof Error ? error.message : 'Unknown error');
  ...
  // Add a new function to fetch a single password when needed (e.g., when viewing)
  const fetchPasswordDetails = async (passwordId: string): Promise<void> => {
    try {
      const token = localStorage.getItem('authToken');
      ...
      if (!response.ok) {
        throw new Error(result.message || 'Failed to fetch password details');
      }
      
      // Update the specific password in the state with its actual password value
      setPasswords(prevPasswords => 
        prevPasswords.map(pwd => 
          pwd._id === passwordId ? {
            ...pwd,
            password: result.data.password,
            // You can also update strength here with actual value if your API provides it
          } : pwd
        )
      );
      
      // Make password visible after fetching
      ...
      console.error('Error fetching password details:', error instanceof Error ? error.message : 'Unknown error');
      // Handle error
    }
  };
  
  // Modify togglePasswordVisibility to fetch the actual password when revealing
  const togglePasswordVisibility = (id: string): void => {
    // If password is currently hidden and trying to show it
    if (!visiblePasswords[id]) {
      // Check if the password is a placeholder
      const passwordObj = passwords.find(p => p._id === id);
      if (passwordObj && passwordObj.password === '••••••••••••••••') {
        // Fetch the actual password before showing
        fetchPasswordDetails(id);
        return;
      }
    }
    
    // Toggle visibility as before for passwords we already have
    setVisiblePasswords(prev => ({
      ...prev,
      [id]: !prev[id],
    }));
  ...
  const handleAddPassword = async () => {
    try {
      ...
      // Create a new password item with the returned data
  ...
  const handleEditPassword = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://password-manager-using-ecc.onrender.com/api/updatePassword/${currentPassword._id}`, {
        ...
        body: JSON.stringify({
          websiteName: currentPassword.websiteName,
          websiteLink: currentPassword.websiteLink,
      ...
      // Calculate strength for UI (since API doesn't return it)
      const strength = calculatePasswordStrength(currentPassword.password);
      
      // Update the password in the state
      setPasswords(prevPasswords => 
        prevPasswords.map(pwd => 
          pwd._id === currentPassword._id ? {
            ...currentPassword,
            strength: strength,
      ...
      // Show success toast using reusable function
  ...
  // Update the handleDeletePassword function in Dashboard.tsx
  const handleDeletePassword = async (id: string) => {
    try {
      ...
      // Show success toast dynamically without extra state
      showToast("Password deleted successfully");
  
      // Close edit modal if it's open for this password
      if (showEditModal && currentPassword && currentPassword._id === id) {
        setShowEditModal(false);
        setCurrentPassword(null);
      }
  
    } catch (error) {
      console.error('Error deleting password:', error instanceof Error ? error.message : 'Unknown error');
  ...
  {showCopiedToast && (
    ...
    >
      ...
      <span className="text-white">
        {copiedText === "Password" && !visiblePasswords[copiedText] 
